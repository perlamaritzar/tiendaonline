      ...
      <table class="table">
        ...
        <tbody>
          @foreach($productos as $producto)
            <tr>
              <th>{{ $producto->id }}</th>
              <td class="imagen"><img src="{{ asset('images/' . $producto->imagen) }}" alt="Imagen de producto"></td>
              ...
              <td class="action-cell"> <!-- Aplicar la clase a la celda de acción -->
                  ...
                  <form action="{{ route('productos.eliminar', $producto->id) }}" method="POST" style="display:inline;">
                      @csrf
                      @method('DELETE')
                      <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('¿Estás seguro de querer eliminar esta producto?')">Eliminar</button>
                  </form>
              </td>                      
            </tr>
          @endforeach
